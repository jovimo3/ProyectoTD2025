...
df <- lapply(seq_along(dt), function(i) {
  ...
  lines_df <- lapply(a[12:end], function(line) {
    cantidad <- str_extract(line, "^\\s{0,2}\\d+")
    precio <- str_extract(line, "\\d+,\\d{2}$")
    importes <- str_extract(line, "(\\d+,\\d{2})\\s*$")
    
    descripcion <- str_trim(
      str_remove_all(line, "^\\s*\\d+\\s*|\\d+,\\d{2}\\s*$")
    )

    return(c(cantidad, descripcion, precio, importes, NA))
  })

  df <- as.data.frame(do.call(rbind, lines_df), stringsAsFactors = FALSE)
  colnames(df) <- c("Cantidad", "Descripcion", "Precio", "Importe", "Peso")
...
for(i in l_compras){
  ...
  for (e in seq_along(i)) {
    line <- strsplit(i[[e]], "\\s{2,}")[[1]]
    
    # Force Table to Have 5 Columns
    if (length(line) < 5) {
      line <- c(line, rep(NA, 5 - length(line)))
    }
    tabl <- append(tabl, list(line))
  }
