...
19.-¿Como ha variado el precio del aceite, y relacionados, a lo largo del tiempo?
...
ggplot(plat_ban, aes(x = Fecha, y = Precio, colour = Descripcion)) + geom_point() + geom_jitter() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + geom_line(aes(group = Descripcion)) + ggtitle("Banana vs Platano")
```
...
Al realizar el calculo, podemos ver que da una medía de 3'37€ en IVA por tiquet.

\nolinenumbers


**Pregunta 19 :¿Como ha variado el precio del aceite, y relacionados, a lo largo del tiempo?**

\linenumbers

```{r figura-precaceite, fig.cap="Variación Precio Aceite"}
...
En el gráfico \@ref(fig: ffigura-precaceite) se puede observar que a lo largo del año las variaciones en el aceite no han sido apenas apreciables excepto en los casos del aceite virgen extra y el aceite virgen, que toman un augmento considerable de casi el doble de su precio prebio en marzo o febrero.
...
#ggplot() + geom_bar(aes(y = df_parking_tiempos$tiempo_diferencia))
